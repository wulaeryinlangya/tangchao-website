    ...
    <div className="console-panel glass-hover rounded-[1.25rem] p-6">
      <div className="flex flex-col gap-6 md:flex-row md:items-center">
        <svg viewBox="0 0 200 200" className="h-52 w-52 shrink-0">
            ...
            return formats.map((f) => {
              ...
              return (
                <path
                  ...
                  fill={f.key === 'business' ? 'rgba(224,149,47,0.55)' : 'rgba(185,130,31,0.2)'}
          ...
          <text x="100" y="104" textAnchor="middle" fontSize="15" fill="rgba(67,42,22,0.9)">
            内容形态
          </text>
          <text x="100" y="122" textAnchor="middle" fontSize="11" fill="rgba(67,42,22,0.55)">
            标签覆盖占比
          </text>
        </svg>
          ...
          {formats.map((f) => (
            <button
              ...
              className={`block w-full rounded-xl border px-4 py-3 text-left transition ${
                active === f.key ? 'border-honey/50 bg-honey/10' : 'border-rule hover:border-[rgba(67,42,22,0.3)]'
              }`}
            >
              <div className="flex items-center justify-between gap-4">
                <span className="font-body text-sm font-medium text-ink">{f.label}</span>
                <span className="font-body text-lg font-semibold text-honey">{f.pct}%</span>
              </div>
              {active === f.key && (
                <p className="mt-1 font-body text-xs font-light text-ink/60">{f.desc}</p>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-rule pt-4">
        ...
        <span className="font-body text-xs uppercase tracking-[0.15em] text-honey">Format</span>
      </div>
    </div>
  )
}
