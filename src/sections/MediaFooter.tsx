  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        <span className="font-body text-xs uppercase tracking-[0.2em] text-ink/50">{t.role}</span>
        <span className="font-body text-xs text-honey">{open ? '收起 ∧' : '展开 ∨'}</span>
      </div>
      <h3 className="mt-2 font-heading text-2xl italic leading-none tracking-[-1px] text-ink">
        {t.title}
      </h3>
      <p className="mt-3 font-body text-sm font-light leading-snug text-ink/80">{t.summary}</p>

      {open && (
        <div className="mt-4 space-y-3 border-t border-rule pt-4">
          <p className="font-body text-sm font-light leading-relaxed text-ink/75">{t.detail}</p>
          <div className="rounded-lg border border-honey/30 bg-honey/5 px-3 py-2 font-body text-xs font-medium text-honey">
            {t.evidence}
          </div>
        </div>
      )}
...
export default function MediaFooter() {
  ...
  return (
    <SectionShell
      ...
      bg={
        <>
          <div className="bg-wash bg-wash--side" />
          <VideoBackdrop
          ...
          <div className="geo-ring" style={{ width: '34rem', height: '34rem', right: '-8rem', bottom: '-10rem' }} />
        </>
      }
    >
      ...
      <footer id="contact" className="mt-16">
        <Reveal>
          <div className="liquid-glass-strong flex flex-col items-center gap-6 rounded-[1.5rem] p-10 text-center md:flex-row md:justify-between md:text-left">
            <div>
              <div className="font-heading text-2xl italic text-ink">欢迎来到糖巢</div>
              <p className="mt-2 font-body text-sm font-light text-ink/70">
                东江畔的创客之乡 · 从老圩镇到省级标杆
              </p>
            </div>
            <div className="flex flex-col items-center gap-2 font-body text-sm font-light text-ink/70 md:items-end">
              <span>公众号：糖巢农文旅</span>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-rule pt-6 font-body text-xs font-light text-ink/50 md:flex-row">
